...
import { ref, toRaw, unref, reactive, h } from 'vue';
import { uploadMyFile } from '/@/api/common/api';
import { Modal } from 'ant-design-vue';
import { createImgPreview } from '/@/components/Preview/index';
import { createWarningModal } from '/@/components/Modal';
...
export function useFileList() {
      ...
      if(file.url){
        try {
          ...
          initViewDomain().then(() => {
            ...
            window.open(onlinePreviewDomain + '?url=' + previewUrl, '_blank');
          });
        } catch (error) {
  ...
  function handleVideoPreview(url) {
    Modal.warning({
      title: '预览提示',
      content: '视频文件暂时不支持在线预览，请联系管理员下载查看',
      okText: '知道了'
    });
  }
  ...
  function handleFileClick(file) {
    const fileUrl = file.url || file.response?.url || file.response?.result?.url;
    if (fileUrl && isVideoFile(fileUrl)) {
